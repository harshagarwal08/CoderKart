    ...
    <section className="text-gray-600 body-font xl:min-h-full min-h-screen">
      <div className="container 2xl:px-16 xl:px-8 lg:px-2 md:px-0 px-8 py-16 w-full mx-auto">
        <div className="flex flex-wrap justify-center">
          {Object.keys(products).length===0 && <p>All the hoodies are currently out of stock. New stock coming soon. Stay Tuned!</p>}
          ...
          <div className="xl:w-1/5 lg:w-1/4 md:w-1/3 sm:w-1/2 w-full p-0 cursor-pointer shadow-xl m-4 transform transiti on duration-500 hover:scale-105 hover:shadow-2xl">
            <a className="block relative rounded overflow-hidden">
              <img alt="ecommerce" className="block h-[40vh] m-auto object-contain" src={products[item].img}/>
            </a>
  ...
  let hoodies = {}
    for(let item of products){
        if(item.title in hoodies){
            if(!hoodies[item.title].color.includes(item.color) && item.availableQty>0) hoodies[item.title].color.push(item.color)
            if(!hoodies[item.title].size.includes(item.size) && item.availableQty>0) hoodies[item.title].size.push(item.size)
        }
        else{
